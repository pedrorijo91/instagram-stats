...
object Instagram {
  ...
  def getAuthUrl(request: Request[AnyContent]): String = {
    ...
    "https://api.instagram.com/oauth/authorize/?client_id=" + Config.clientId + "&redirect_uri=" + path + "&response_type=code" + "&scope=" + scopes
  }
  ...
  def getAccessToken(request: Request[_], code: String): Option[String] = {
    ...
    If you want to check stats for your own account just get an access token and set it as an environment variable.
    The environment variable will be read by the Config object
    ...
    Config.accessToken.fold {
      implicit val context = scala.concurrent.ExecutionContext.Implicits.global

      val path = (if (request.host.contains(":9000")) "http://" else "https://") + request.host + redirectURL

      //TODO 2 seconds timeout
      val postParams: Map[String, Seq[String]] = Map("client_id" -> Seq(Config.clientId), "client_secret" -> Seq(Config.clientSecret),
      ...
      case accessToken => Some(accessToken)
    }
